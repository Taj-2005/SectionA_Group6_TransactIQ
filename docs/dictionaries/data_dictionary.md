...
- **Dataset variants**: <!-- e.g., WildChat-1M vs WildChat-4.8M (if used as backup) -->
- **Raw format**: <!-- e.g., HF dataset (streaming) parquet/JSON as provided by Hugging Face -->
- **Refresh**: <!-- e.g., static snapshot; record the date accessed -->
- **Grain (row-level meaning)**: <!-- e.g., one row = one message-turn record -->
- **Primary keys**: <!-- e.g., conversation_id + turn_id -->
- **Time window**: <!-- start date → end date (from timestamps) -->
